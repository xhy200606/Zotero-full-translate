(function () {
    ...
    function brandIconNode(doc, className = "") {
        const wrap = doc.createElement("span");
        wrap.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" aria-hidden="true" focusable="false"><rect width="64" height="64" rx="18" fill="#EADDFF"/><g transform="translate(8 8)"><path d="M7 9.5A5.5 5.5 0 0 1 12.5 4h17A5.5 5.5 0 0 1 35 9.5v23A5.5 5.5 0 0 1 29.5 38h-17A5.5 5.5 0 0 1 7 32.5v-23Z" fill="#DACFE9" stroke="#6750A4" stroke-width="1.4"/><path d="M16 15.5a5.5 5.5 0 0 1 5.5-5.5h14a5.5 5.5 0 0 1 5.5 5.5v23a5.5 5.5 0 0 1-5.5 5.5h-14a5.5 5.5 0 0 1-5.5-5.5v-23Z" fill="#FFFFFF" stroke="#6750A4" stroke-width="1.6"/><g fill="none" stroke="#6750A4" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M13 13h12M13 18h8M23 21h11M23 26h11M23 31h8"/><path d="M10.5 27.5h10m-3.5-3.5 3.5 3.5L17 31m20.5 4.5h-10m3.5-3.5-3.5 3.5L31 39"/></g></g></svg>`;
    ...
    const Addon = {
        ...
        cloudAutoSyncing: new Set(),
        cloudAutoSynced: new Set(),
        cloudRecoveryTimer: null,
        cloudProviderPoolTimer: null,
        cloudProviderPoolUpdatedAt: 0,
        log(...args) {
            if (!this.pref("debug", false))
                return;
            try {
        ...
        async init({ id, version, rootURI }) {
            ...
            if (this.pref("cloud.thinClient", true))
                this.setPref("output.layoutEngine", "zft-cloud");
            this.cloudClientID();
            ...
            if (asString(this.pref("cloud.baseURL", "")).trim() && asString(this.pref("cloud.apiKey", "")).trim())
                setTimeout(() => this.refreshCloudProviderPool(true).catch(() => {}), 700);
        },
        ...
        async shutdown() {
            ...
            try {
                if (this.viewContextHandler)
                    Zotero.Reader.unregisterEventListener("createViewContextMenu", this.viewContextHandler);
            }
            catch (_) { }
            if (this.cloudRecoveryTimer)
                clearTimeout(this.cloudRecoveryTimer);
            this.cloudRecoveryTimer = null;
            if (this.cloudProviderPoolTimer)
                clearInterval(this.cloudProviderPoolTimer);
            this.cloudProviderPoolTimer = null;
            for (const state of this.states.values())
                this.destroyState(state);
            this.states.clear();
            for (const [win, handler] of this.windowHandlers) {
                try {
                    win.removeEventListener("keydown", handler, true);
                }
                catch (_) { }
            ...
            for (const timer of this.toolbarSweepTimers.values())
                clearInterval(timer);
        ...
        onRenderToolbar({ reader, doc, append }) {
            ...
            this.scheduleNativeCompareRestore(reader, doc);
            setTimeout(() => this.refreshCloudProviderPool(false).catch(() => {}), 250);
            setTimeout(() => this.syncCloudTranslationForReader(reader).catch((e) => this.log("cloud cross-device sync", this.safeErrorMessage(e))), 900);
        },
        createToolbarButton(reader, doc) {
            ...
            button.title = "Zotero Full Translate（Shift+T）";
            const icon = brandIconNode(doc, "zft-toolbar-brand-icon");
            const badge = doc.createElement("span");
            badge.className = "zft-toolbar-badge";
            badge.hidden = true;
            button.append(icon, badge);
            button.setAttribute("aria-label", "Zotero Full Translate");
        ...
        ensureReaderToolbarButton(reader) {
            if (!reader || !this.pref("enabled", true))
                return false;
            ...
            if (existing) {
                ...
                this.scheduleNativeCompareRestore(reader, doc);
                this.maintainNativeCompareSession(reader);
                return true;
            }
        ...
        startToolbarSweep(win) {
            if (!win || this.toolbarSweepTimers.has(win))
                return;
            const tick = () => {
                try {
                    const reader = this.getActiveReader(win);
                    if (reader)
                        this.ensureReaderToolbarButton(reader);
                }
                catch (e) {
        ...
        .zft-toolbar-button{appearance:none;border:1px solid transparent;background:transparent;color:inherit;border-radius:8px;width:32px;min-width:32px;height:30px;cursor:pointer;padding:3px;position:relative;display:inline-flex;align-items:center;justify-content:center;transition:background .12s ease,border-color .12s ease}
        .zft-toolbar-button:hover{background:color-mix(in srgb,currentColor 8%,transparent);border-color:color-mix(in srgb,currentColor 10%,transparent)}
        .zft-toolbar-button .zft-toolbar-brand-icon{display:block;width:24px;height:24px;border-radius:7px;overflow:visible}
        .zft-toolbar-button[data-quota-warning="low"] .zft-toolbar-badge{background:#d97706}
        .zft-toolbar-button[data-quota-warning="critical"] .zft-toolbar-badge{background:#b42318}
        .zft-toolbar-button .zft-toolbar-badge{position:absolute;right:-5px;top:-5px;min-width:15px;height:15px;padding:0 3px;box-sizing:border-box;border-radius:999px;background:#6750A4;color:#fff;border:1.5px solid var(--material-background,#fff);font-size:8px;font-weight:750;line-height:12px;display:flex;align-items:center;justify-content:center;font-variant-numeric:tabular-nums}
        .zft-toolbar-button .zft-toolbar-badge[hidden]{display:none}
        ...
        .zft-quick-menu{position:fixed;z-index:99999;width:318px;max-width:calc(100vw - 20px);box-sizing:border-box;background:var(--material-background,#fff);color:var(--fill-primary,#222);border:1px solid color-mix(in srgb,currentColor 16%,transparent);border-radius:14px;padding:7px;box-shadow:0 14px 42px rgba(0,0,0,.20);max-height:calc(100vh - 18px);overflow:auto}
        .zft-quick-menu .zft-menu-head{display:flex;align-items:center;gap:10px;padding:10px 10px 9px;border-bottom:1px solid color-mix(in srgb,currentColor 11%,transparent);margin-bottom:5px}
        .zft-quick-menu .zft-menu-logo{display:flex;align-items:center;justify-content:center;width:34px;height:34px;flex:0 0 34px;border-radius:10px;overflow:hidden}.zft-quick-menu .zft-menu-logo svg{display:block;width:34px;height:34px;overflow:visible}
        .zft-quick-menu .zft-menu-brand{min-width:0;flex:1}
        .zft-quick-menu .zft-menu-title{font-size:13px;font-weight:720;line-height:1.25}
        .zft-quick-menu .zft-menu-version{font-size:10px;opacity:.55;margin-top:2px;font-variant-numeric:tabular-nums}
        .zft-quick-menu .zft-menu-state{font-size:10px;font-weight:650;padding:3px 7px;border-radius:999px;background:color-mix(in srgb,currentColor 7%,transparent);white-space:nowrap}
        .zft-quick-menu .zft-menu-summary{padding:5px 10px 7px;font-size:11px;line-height:1.45;opacity:.66}
        .zft-quick-menu .zft-menu-quota-warning{margin:3px 7px 6px;padding:8px 9px;border-radius:9px;background:#fff4e5;color:#8a4b08;border:1px solid #f3c77a;font-size:10.5px;font-weight:650;line-height:1.4}
        .zft-quick-menu .zft-menu-quota-warning[data-level="critical"]{background:#fff0ee;color:#9b1c13;border-color:#efb3ad}
        .zft-quick-menu .zft-menu-quota-warning[hidden]{display:none}
        .zft-quick-menu .zft-menu-group{padding:4px 3px 2px}
        .zft-quick-menu .zft-menu-group-title{padding:4px 7px 3px;font-size:10px;font-weight:700;letter-spacing:.03em;opacity:.48}
        .zft-quick-menu button.zft-menu-action{display:flex;width:100%;align-items:center;gap:10px;text-align:left;border:0;background:transparent;color:inherit;border-radius:9px;padding:8px 8px;cursor:pointer}
        .zft-quick-menu button.zft-menu-action:hover{background:color-mix(in srgb,currentColor 8%,transparent)}
        .zft-quick-menu button.zft-menu-action[data-primary="true"]{background:color-mix(in srgb,currentColor 7%,transparent)}
        .zft-quick-menu button.zft-menu-action[data-primary="true"]:hover{background:color-mix(in srgb,currentColor 11%,transparent)}
        .zft-quick-menu .zft-menu-icon{display:flex;align-items:center;justify-content:center;width:30px;height:30px;flex:0 0 30px;border-radius:8px;background:color-mix(in srgb,currentColor 7%,transparent);font-size:14px;font-weight:720}
        .zft-quick-menu .zft-menu-action[data-primary="true"] .zft-menu-icon{background:var(--material-background,#fff);box-shadow:0 1px 3px rgba(0,0,0,.09)}
        .zft-quick-menu .zft-menu-copy{display:flex;flex-direction:column;min-width:0;flex:1}
        .zft-quick-menu .zft-menu-label{font-size:12px;font-weight:650;line-height:1.3}
        .zft-quick-menu .zft-menu-desc{font-size:10px;opacity:.58;line-height:1.35;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .zft-quick-menu .zft-menu-tail{font-size:11px;opacity:.55;white-space:nowrap}
        .zft-quick-menu .zft-menu-sep{height:1px;background:color-mix(in srgb,currentColor 10%,transparent);margin:5px 6px}
        .zft-quick-menu button:disabled{opacity:.42;cursor:default}
        .zft-quick-menu button.zft-menu-action[data-danger="true"] .zft-menu-label{color:#b42318}
        .zft-toolbar-button[data-running="true"]{width:32px;min-width:32px}
      ...
      `;
        ...
        refreshStateDocument(state, preferredDoc = null) {
            ...
            if (!doc) {
                ...
                if (this.isLiveDocument(fresh))
                    doc = fresh;
            }
            if (!doc && this.isLiveDocument(state.doc))
                doc = state.doc;
            if (!doc)
                return null;
            if (state.doc !== doc || !this.isLiveDocument(state.doc)) {
                ...
                state.nativeCompareRestorePending = false;
                state.nativeCompareRestoreAttempted = false;
                state.nativeCompareRestoreAttempts = 0;
                state.nativeCompareRestoreDeadline = 0;
        ...
        layoutEngineLabel(engine) {
            if (engine === "doc2x")
                return "Doc2X";
            if (engine === "zft-cloud" || engine === "pdf2zh-cloud")
                return "Zotero Full Translate Cloud";
        ...
        normalizedLayoutEngine(value = null) {
            ...
            if (raw === "pdf2zh-cloud")
                return "zft-cloud";
            if (["doc2x", "pdf2zh", "zft-cloud"].includes(raw))
                return raw;
            return "zft-cloud";
        },
        getState(reader, doc) {
            ...
            if (!state) {
                state = {
                    ...
                    cloudDocumentState: "IDLE",
                    cloudDocumentStateDetail: "",
                    taskScope: "all",
                    nativeCompareRestorePending: false,
                    nativeCompareRestoreAttempted: false,
                    nativeCompareRestoreAttempts: 0,
                    nativeCompareRestoreDeadline: 0,
                };
        ...
        showQuickMenu(reader, doc, anchor) {
            ...
            const connected = !!asString(this.pref("cloud.baseURL", "")).trim() && !!asString(this.pref("cloud.apiKey", "")).trim();
            const cloudSummary = this.cloudProviderSummary();

            const head = doc.createElement("div");
            head.className = "zft-menu-head";
            const logo = doc.createElement("div");
            logo.className = "zft-menu-logo";
            logo.append(brandIconNode(doc, "zft-menu-brand-icon"));
            const brand = doc.createElement("div");
            brand.className = "zft-menu-brand";
            const title = doc.createElement("div");
            title.className = "zft-menu-title";
            title.textContent = "Zotero Full Translate";
            const version = doc.createElement("div");
            version.className = "zft-menu-version";
            version.textContent = `插件 ${this.version || "0.4.2"} · Cloud 2.5.2`;
            brand.append(title, version);
            const stateChip = doc.createElement("div");
            stateChip.className = "zft-menu-state";
            stateChip.textContent = state.running ? `${Math.round(state.taskProgress || 0)}%` : (connected ? "已连接" : "未连接");
            head.append(logo, brand, stateChip);
            menu.append(head);

            const summary = doc.createElement("div");
            summary.className = "zft-menu-summary";
            summary.textContent = state.running
                ? `${state.taskStatusText || "正在翻译"} · ${Math.round(state.taskProgress || 0)}%`
                : cloudSummary;
            menu.append(summary);
            const quotaWarning = doc.createElement("div");
            quotaWarning.className = "zft-menu-quota-warning";
            const updateQuotaWarning = () => {
                const level = this.cloudQuotaWarningLevel();
                const text = this.cloudQuotaWarningText();
                quotaWarning.hidden = !level;
                if (level) {
                    quotaWarning.dataset.level = level;
                    quotaWarning.textContent = `⚠ API 低额度 · ${text}`;
                }
                else {
                    delete quotaWarning.dataset.level;
                    quotaWarning.textContent = "";
                }
            };
            updateQuotaWarning();
            menu.append(quotaWarning);
            if (!state.running && connected) {
                this.refreshCloudProviderPool(true).then(() => {
                    if (menu.isConnected) {
                        summary.textContent = this.cloudProviderSummary();
                        updateQuotaWarning();
                    }
                }).catch(() => {});
            }
            ...
            const addGroup = (label) => {
                ...
                group.className = "zft-menu-group";
                const groupTitle = doc.createElement("div");
                groupTitle.className = "zft-menu-group-title";
                groupTitle.textContent = label;
                group.append(groupTitle);
                menu.append(group);
                return group;
            };
            const addAction = (group, { label, icon = "•", tail = "›", fn, disabled = false, danger = false, primary = false }) => {
                const b = doc.createElement("button");
                b.type = "button";
                b.className = "zft-menu-action";
                b.disabled = disabled;
                if (danger)
                    b.dataset.danger = "true";
                if (primary)
                    b.dataset.primary = "true";
                const iconNode = doc.createElement("span");
                iconNode.className = "zft-menu-icon";
                iconNode.textContent = icon;
                const copy = doc.createElement("span");
                copy.className = "zft-menu-copy";
                const labelNode = doc.createElement("span");
                labelNode.className = "zft-menu-label";
                labelNode.textContent = label;
                copy.append(labelNode);
                const tailNode = doc.createElement("span");
                tailNode.className = "zft-menu-tail";
                tailNode.textContent = tail;
                b.append(iconNode, copy, tailNode);
            ...
            const translation = addGroup("翻译");
            addAction(translation, {
                label: state.running ? "全文翻译进行中" : "翻译全文",
                icon: state.running ? "…" : "文",
                tail: state.running ? `${Math.round(state.taskProgress || 0)}%` : "›",
                fn: () => this.exportLayoutPDF(reader, "zft-cloud"),
            ...
            addAction(translation, {
                label: "重新翻译",
                icon: "↻",
                fn: () => this.exportLayoutPDF(reader, "zft-cloud", { forceRetranslate: true }),
                disabled: state.running,
            });
            if (state.running) {
                addAction(translation, {
                    ...
                    icon: "×",
                    tail: "",
                    fn: () => this.cancelTranslation(state),
                    danger: true,
                });
            }

            sep();
            const tasks = addGroup("任务");
            if (state.running) {
                addAction(tasks, {
                    label: "显示任务进度",
                    icon: "◔",
            ...
            addAction(tasks, {
                label: "检查云端任务",
                icon: "✓",
                fn: () => this.restoreCloudJobs({ notify: true }),
            });
            addAction(tasks, {
                label: "打开 Cloud 用户中心",
                icon: "☁",
                fn: () => this.openCloudConsole(),
            });

            sep();
            const reading = addGroup("阅读");
            ...
            addAction(reading, {
                label: comparisonOpen ? "关闭原文 / 译文对照" : "打开原文 / 译文对照",
                icon: "◫",
                fn: () => {
                    if (state.nativeCompare)
                        this.stopNativeCompareSession(state);
                    else if (this.safeDOMConnected(state.cloudPanel))
                        this.closeCloudPDFPanel(state);
                    else
                        return this.openComparisonForReader(reader);
                },
                disabled: !comparePair && !comparisonOpen,
            });
            addAction(reading, {
                label: "联动滚动",
                icon: "⇅",
                tail: this.pref("compare.syncScroll", true) ? "开" : "关",
                fn: () => {
            ...
            if (!this.pref("cloud.thinClient", true)) {
                sep();
                const legacy = addGroup("兼容模式");
                addAction(legacy, {
                    label: "阅读器全文预览",
                    icon: "旧",
                    fn: () => this.translateReader(reader, doc, { scope: "all", force: state.completed && state.taskScope === "all" }),
                    disabled: state.running,
                });
                addAction(legacy, {
                    label: "翻译当前页",
                    icon: "页",
                    fn: () => this.translateReader(reader, doc, { scope: "current-page" }),
                    disabled: state.running,
                });
            }
            ...
            if (rect) {
                const menuWidth = 318;
                menu.style.left = `${Math.max(8, Math.min(doc.defaultView.innerWidth - menuWidth - 8, rect.right - menuWidth))}px`;
                menu.style.top = `${Math.max(8, Math.min(doc.defaultView.innerHeight - 560, rect.bottom + 5))}px`;
            }
            else {
        ...
        showQuotaDiagnostic(serviceID = null) {
            ...
            try {
                Services.prompt.alert(Zotero.getMainWindow?.() || null, "全文翻译 · 引擎额度", lines);
            }
            catch (_) {
                this.notify("引擎额度", lines);
            }
        },
        closeTranslationPanel(state, { remove = true } = {}) {
            ...
            let panel = null;
            try {
                panel = doc?.getElementById(PANEL_ID) || (this.safeDOMConnected(state.panel) ? state.panel : null);
            }
            catch (_) { }
            try {
        ...
        dismissTaskHUD(state) {
            ...
            let hud = null;
            try {
                hud = doc?.getElementById(TASK_HUD_ID) || (this.safeDOMConnected(state.taskHUD) ? state.taskHUD : null);
            }
            catch (_) { }
            try {
                hud?.remove();
        ...
        bindPanelControls(state, panel) {
            ...
            try {
                ...
                if (note)
                    note.onclick = () => this.exportNote(state.reader).catch((e) => this.reportError(e, state.reader));
                const retranslate = panel.querySelector('[data-action="retranslate"]');
                if (retranslate)
                    retranslate.onclick = () => this.retranslateCurrentReader(state.reader).catch((e) => this.reportError(e, state.reader));
        ...
        ensurePanel(state, title = "全文翻译") {
            ...
            if (!doc)
                throw new Error("无法访问 Zotero 阅读器文档。请关闭并重新打开 PDF 后再试。");
            this.injectReaderStyle(doc);
            let panel = null;
            try {
        ...
        applyPanelLayout(state, visible) {
            ...
            let split = null;
            try {
                split = doc?.getElementById("split-view");
            }
            catch (_) { }
            if (!split)
                return;
            if (!state.originalSplitStyle) {
                try {
                    state.originalSplitStyle = { width: split.style.width, height: split.style.height };
                }
                catch (_) {
                    state.originalSplitStyle = { width: "", height: "" };
                }
            }
            try {
                if (!visible) {
        ...
        async retranslateCurrentReader(reader) {
            if (!reader)
                throw new Error("无法识别当前 Zotero 阅读器。");
            if (this.pref("cloud.thinClient", true)) {
        ...
        closeCloudPDFPanel(state) {
            ...
            let panel = null;
            try {
                panel = doc?.getElementById?.(CLOUD_PANEL_ID) || (this.safeDOMConnected(state.cloudPanel) ? state.cloudPanel : null);
            }
            catch (_) { }
            try {
                panel?.remove();
        ...
        async openCloudPDFPanel(reader, sourceItem, translatedItem) {
            if (!reader || !sourceItem?.id || !translatedItem?.id)
                throw new Error("原文或译文附件不可用。");
            const translatedPath = await translatedItem.getFilePathAsync();
            if (!translatedPath || !(await IOUtils.exists(translatedPath)))
                throw new Error("本地译文 PDF 不存在。");
            const state = this.getState(reader, this.getReaderDoc(reader));
            const doc = this.refreshStateDocument(state);
            if (!doc)
                throw new Error("无法访问 Zotero 阅读器文档。");
            this.injectReaderStyle(doc);
            try {
                this.stopNativeCompareSession(state);
            }
          ...
          <div class="zft-cloud-body">
            ...
            if (titleNode)
                titleNode.textContent = `译文 · ${asString(translatedItem.getField?.("title") || "PDF")}`;
            ...
            if (close)
                close.onclick = () => this.closeCloudPDFPanel(state);
            const tab = panel.querySelector('[data-action="tab"]');
            if (tab)
                tab.onclick = () => Zotero.Reader.open(translatedItem.id);
            const retranslate = panel.querySelector('[data-action="retranslate"]');
            if (retranslate)
                retranslate.onclick = () => this.exportLayoutPDF(reader, "zft-cloud", { forceRetranslate: true }).catch((e) => this.reportError(e, reader));
            if (frame) {
                frame.onerror = () => { try {
                ...
                try {
                    ...
                    setTimeout(() => {
                        try {
                            if (!this.safeDOMConnected(frame))
                                return;
        ...
        async secondaryPDFReady(view, timeoutMs = 4500) {
            ...
            while (Date.now() - started < timeoutMs) {
                try {
                    ...
                    if (app?.pdfDocument && Number(app.pdfDocument.numPages) > 0)
                        return true;
        ...
        setStatus(state, text, progress) {
            ...
            if (typeof progress === "number")
                state.taskProgress = Math.max(0, Math.min(100, progress));
            this.refreshStateDocument(state);
            try {
                if (this.safeDOMConnected(state.status))
                    state.status.textContent = state.taskStatusText;
                else
                    state.status = null;
            }
            catch (_) {
            ...
            try {
                if (this.safeDOMConnected(state.progress) && typeof progress === "number")
                    state.progress.style.width = `${state.taskProgress}%`;
                else if (!this.safeDOMConnected(state.progress))
                    state.progress = null;
            }
            catch (_) {
                state.progress = null;
            ...
            try {
                if (state.running || state.taskHUD)
                    this.updateTaskHUD(state);
            }
            catch (_) { }
            try {
            ...
            try {
                state.parserTestCallback?.({ text: state.taskStatusText, progress: state.taskProgress });
            }
            catch (_) { }
        },
        ensureTaskHUD(state) {
            if (state?.taskHUDDismissed)
                return null;
            const doc = this.refreshStateDocument(state);
            if (!doc)
                return null;
            ...
            let hud = null;
            try {
                hud = doc.getElementById(TASK_HUD_ID);
            }
            catch (_) { }
            if (!hud) {
          ...
          <div class="zft-task-main">
            ...
            <div class="zft-task-copy">
              <div class="zft-task-title">全文翻译任务</div>
          ...
          <div class="zft-task-progress"><i></i></div>`;
            ...
            try {
                ...
                if (cancel)
                    cancel.onclick = () => this.cancelTranslation(state);
                const detail = hud.querySelector(".zft-task-detail");
                if (detail)
                    detail.onclick = () => this.showTaskDiagnostic(state);
                const dismiss = hud.querySelector(".zft-task-dismiss");
                if (dismiss)
                    dismiss.onclick = () => this.dismissTaskHUD(state);
            }
            catch (_) { }
            try {
                hud.hidden = false;
        ...
        updateTaskHUD(state, explicitState = null) {
            ...
            try {
                ...
                const detail = hud.querySelector(".zft-task-detail");
                if (title)
                    title.textContent = state.taskScope === "current-page" ? "当前页翻译任务" : "全文翻译任务";
                if (statusNode)
                    statusNode.textContent = state.running ? `${status} · ${elapsedText}` : status;
                if (pctNode)
                ...
                if (detail)
                    detail.hidden = !state.taskErrorReport;
            }
            catch (_) {
                state.taskHUD = null;
            }
        ...
        updateToolbarTaskState(state) {
            const pct = Math.round(Number(state?.taskProgress) || 0);
            const quotaLevel = this.cloudQuotaWarningLevel();
            const quotaText = this.cloudQuotaWarningText();
            for (const button of [...(state?.toolbarButtons || [])]) {
                try {
                    ...
                    const badge = button.querySelector?.(".zft-toolbar-badge");
                    if (state.running) {
                        button.dataset.running = "true";
                        delete button.dataset.quotaWarning;
                        if (badge) {
                            badge.hidden = false;
                            badge.textContent = pct > 0 ? String(pct) : "…";
                        }
                        ...
                        if (quotaLevel)
                            button.dataset.quotaWarning = quotaLevel;
                        else
                            delete button.dataset.quotaWarning;
                        if (badge) {
                            badge.hidden = !(quotaLevel || state.completed);
                            badge.textContent = quotaLevel ? "!" : (state.completed ? "✓" : "");
                        }
                        button.title = quotaLevel
                            ? `API 低额度警告：${quotaText}`
                            : (state.completed ? "翻译已完成（点击打开菜单）" : "Zotero Full Translate（Shift+T）");
                    }
                }
                catch (_) {
                    try {
        ...
        finishTask(state, kind = "done", message = "翻译完成") {
            ...
            this.updateToolbarTaskState(state);
            if (kind === "done") {
                setTimeout(() => this.refreshCloudProviderPool(true).catch(() => {}), 350);
                setTimeout(() => {
                    try {
                        if (this.safeDOMConnected(state.taskHUD) && !state.running)
                            state.taskHUD.hidden = true;
                    }
                    catch (_) {
                        state.taskHUD = null;
                    }
        ...
        buildNetworkDiagnostic(stage, error, url = "", extra = {}) {
            ...
            if (extra.requestContentType !== undefined)
                lines.push(`请求 Content-Type：${extra.requestContentType || "<空>"}`);
            if (extra.apiCode !== undefined && extra.apiCode !== null)
                lines.push(`MinerU code：${extra.apiCode}`);
            if (extra.apiMessage)
                lines.push(`MinerU msg：${extra.apiMessage}`);
            if (extra.traceID)
                lines.push(`MinerU trace_id：${extra.traceID}`);
            if (responseText)
                lines.push(`响应：${responseText}`);
            if (extra.hint)
                lines.push(`建议：${extra.hint}`);
            return lines.join("\n");
        },
        ...
        getCurrentPageIndex(stateOrReader) {
            ...
            try {
                const n = Number(reader?._primaryView?._viewState?.pageIndex
        ...
        async translateReader(reader, doc, options = {}) {
            ...
            try {
                ...
                const itemID = this.getAttachmentID(reader);
                if (!itemID)
                    throw new Error("无法识别当前阅读器中的附件。");
                const item = Zotero.Items.get(itemID);
                if (!item?.isAttachment?.())
                    throw new Error("当前阅读器项目不是附件。");
                const path = await item.getFilePathAsync();
                if (!path || !/\.pdf$/i.test(path))
                    throw new Error("当前附件不是可访问的 PDF 文件。");
                ...
                if (!segments.length)
                    throw new Error(scope === "current-page" ? "当前页没有可翻译文本。" : "未能从 PDF 中提取可翻译文本。");
                ...
                state.completed = true;
                if (scope === "all")
                    await this.saveTranslationCache(state);
                ...
                if (errorName === "ZFTCancelled") {
                    try {
                        this.finishTask(state, "idle", "翻译已取消");
                    }
                    catch (_) {
                        state.running = false;
                    }
                ...
                try {
                    this.finishTask(state, "error", zftStage ? `${zftStage}失败 · 点击“详情”` : `失败：${errorMessage}`);
                }
                catch (_) {
                    state.running = false;
                    if (state.taskTimer)
        ...
        async parseDocumentWithParser(parser, item, path, state) {
            if (parser === "mineru-api")
                return this.parseWithMinerUAPI(item, path, state);
            if (parser === "mineru-local")
                return this.parseWithMinerULocal(item, path, state);
            if (parser === "doc2x-md")
                return this.parseWithDoc2X(item, path, state);
        ...
        async parseWithZotero(item, path, state) {
            ...
            if (!result?.text)
                throw new Error("Zotero PDFWorker 未返回文本。扫描版 PDF 建议改用 MinerU。 ");
        ...
        async parseWithMinerULocal(item, path, state) {
            ...
            if (!md && !layoutBlocks.length)
                throw new Error("MinerU 已运行，但未找到 Markdown 或 content_list JSON 输出。");
        ...
        async parseWithMinerUAPI(item, path, state) {
            ...
            if (!md && !layoutBlocks.length)
                throw new Error("MinerU 结果中未找到可用的 Markdown 或 content_list JSON。");
        ...
        makeMinerUParsed(source, markdown, layoutBlocks) {
            if (layoutBlocks?.length) {
                ...
                for (const block of layoutBlocks) {
                    if (!grouped.has(block.pageIndex))
                        grouped.set(block.pageIndex, []);
        ...
        normalizeMinerUContentList(data) {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (!Number.isInteger(pageIndex) || !bbox)
                    continue;
                const type = asString(block.type, "text").toLowerCase();
                let text = asString(block.text, "").trim();
                if (!text && Array.isArray(block.list_items))
                    text = block.list_items.map((x) => asString(x).trim()).filter(Boolean).join("\n");
        ...
        extractMinerUV2Text(value) {
            ...
            if (Array.isArray(value))
                return value.map((x) => this.extractMinerUV2Text(x)).filter(Boolean).join(" ");
            ...
            for (const key of preferred)
                if (value[key] !== undefined)
                    parts.push(this.extractMinerUV2Text(value[key]));
            if (Array.isArray(value.list_items))
                parts.push(this.extractMinerUV2Text(value.list_items));
        ...
        normalizeMinerUBBox(bbox) {
            ...
            if (max <= 1.5)
                [x0, y0, x1, y1] = [x0, y0, x1, y1].map((v) => v * 1000);
        ...
        async parseWithDoc2X(item, path, state) {
            ...
            if (!mdPath)
                throw new Error("Doc2X 已运行，但未找到 Markdown 输出。");
        ...
        prepareSegments(parsed, itemID) {
            ...
            for (const page of parsed.pages || []) {
                ...
                for (let text of pieces) {
                    ...
                    if (ignoreSubtitles && this.looksLikeHeading(text))
                        continue;
        ...
        prepareLayoutSegments(parsed, itemID) {
            ...
            for (const block of parsed.layoutBlocks) {
                if (block.protectedBlock)
                    continue;
                if (block.auxiliary && !translateAuxiliary)
                    continue;
                const text = asString(block.text).replace(/\u0000/g, "").trim();
                if (!text)
                    continue;
                if (block.semanticRole === "author" && !!this.pref("structure.preserveAuthors", true))
                    continue;
                if (ignoreRefs && (block.isReference || this.isReferenceHeading(text))) {
                    referencesStarted = true;
                    continue;
                }
                if (ignoreRefs && referencesStarted)
                    continue;
                if (ignoreSubtitles && (block.textLevel > 0 || this.looksLikeHeading(text)))
                    continue;
                if (!this.shouldTranslateText(text))
                    continue;
        ...
        headingLevelFromText(text) {
            ...
            if (/^(chapter|section)\s+\d+/i.test(t))
                return 1;
            return 0;
        },
        looksLikeAuthorLine(text) {
            const t = asString(text).trim();
            if (!t || t.length > 220 || /[.!?。！？]{2,}/.test(t))
                return false;
            if (/\b(university|institute|department|school|laboratory|lab\b|college|hospital|academy|center|centre)\b/i.test(t))
                return false;
            if (/@|https?:|doi\b/i.test(t))
                return false;
        ...
        annotateSemanticRoles(layoutBlocks) {
            ...
            if (!blocks.length || !this.pref("structure.semanticStyles", true))
                return blocks;
            ...
            for (const b of firstPageText) {
                ...
                if (y0 > 360 || t.length < 5 || t.length > 420)
                    continue;
                if (/^(abstract|摘要|keywords?|关键词)\b/i.test(t))
                    continue;
                const width = x1 - x0;
                const height = y1 - y0;
                let score = 0;
                ...
                if (this.headingLevelFromText(t))
                    score -= 35;
                if (score > titleScore) {
                    paperTitle = b;
                    titleScore = score;
        ...
        looksLikeHeading(text) {
            ...
            if (/^[A-Z][A-Z\s\-:]{3,}$/.test(t))
                return true;
            if (/^#{1,6}\s*/.test(asString(text).trim()))
                return true;
            return false;
        },
        ...
        resolveCaptionAnchor(layoutBlocks, blockLike) {
            if (!blockLike || !Array.isArray(blockLike.bbox))
                return null;
        ...
        enrichSegmentLayoutMetadata(state) {
            ...
            for (const seg of state.segments) {
                ...
                if (looksCaption && anchor) {
                    ...
                    if (anchor.block.type === "table")
                        seg.semanticRole = "table-caption";
                    else if (["image", "chart"].includes(anchor.block.type))
                        seg.semanticRole = "figure-caption";
                    else if (!/caption$/.test(seg.semanticRole || ""))
                        seg.semanticRole = "caption";
                }
            }
        },
        ...
        recordTranslationUsage(serviceID, chars, requests = 1) {
            if (!this.pref("quota.enabled", true))
                return;
            const config = this.getQuotaConfig(serviceID);
            const periodKey = this.quotaPeriodKey(config.period);
            const store = this.readJSONPref("quota.usage");
            if (!store[config.serviceID])
                store[config.serviceID] = {};
        ...
        resetQuotaUsage(serviceID) {
            ...
            const store = this.readJSONPref("quota.usage");
            if (store?.[config.serviceID])
                delete store[config.serviceID][periodKey];
            this.writeJSONPref("quota.usage", store);
        ...
        formatQuotaNumber(value) {
            ...
            if (n >= 100000000)
                return `${(n / 100000000).toFixed(n % 100000000 ? 1 : 0)}亿`;
            if (n >= 10000)
                return `${(n / 10000).toFixed(n % 10000 ? 1 : 0)}万`;
        ...
        quotaSummaryText(serviceID = null) {
            ...
            const parts = [];
            if (q.charsLimit)
                parts.push(`${this.formatQuotaNumber(q.chars)}/${this.formatQuotaNumber(q.charsLimit)}字符`);
            if (q.requestsLimit)
                parts.push(`${this.formatQuotaNumber(q.requests)}/${this.formatQuotaNumber(q.requestsLimit)}次`);
            if (!parts.length)
                parts.push(`${this.formatQuotaNumber(q.chars)}字符 · ${this.formatQuotaNumber(q.requests)}次（本地统计）`);
        ...
        quotaPreflight(state, segments) {
            ...
            if (!(over || near) || !this.pref("quota.confirmNearLimit", true))
                return true;
            const lines = [
        ...
        getEffectiveRateSettings() {
            ...
            if (quota?.qps)
                qps = Math.min(qps, Math.max(0.1, Number(quota.qps) || qps));
        ...
        async waitForTranslationRateSlot(state) {
            ...
            try {
                ...
                if (next > now)
                    await this.cancellableDelay(state, next - now);
            ...
            while (true) {
                ...
                try {
                    ...
                    if (e?.name === "ZFTCancelled")
                        throw e;
                    if (!rate.enabled || !this.isRateLimitError(e) || attempt >= rate.maxRetries)
                        throw e;
        ...
        async translateSegments(state, item) {
            ...
            if (rate.enabled)
                concurrency = Math.min(concurrency, rate.maxConcurrent);
            let cursor = 0;
            let completed = 0;
            const pageTotals = new Map();
            const pageDone = new Map();
            for (const seg of state.segments)
                pageTotals.set(seg.pageIndex, (pageTotals.get(seg.pageIndex) || 0) + 1);
            const totalPages = pageTotals.size;
            if (rate.enabled)
                this.setStatus(state, `翻译限速：${rate.qps} QPS · 最大 ${concurrency} 并发`, 8);
            const workers = Array.from({ length: Math.min(concurrency, total) }, async () => {
                while (true) {
                    ...
                    if (state.displayMode === "translation" && state.layoutAvailable)
                        this.scheduleOverlayRender(state);
        ...
        async translateText(text, itemID, state = null) {
            const provider = asString(this.pref("translationProvider", "pdftranslate"));
            if (provider === "gpt")
                return this.translateWithGPT(text, state);
        ...
        async translateWithPDFTranslate(text, itemID) {
            ...
            if (usedService && usedService !== "pdftranslate-default")
                this.setPref("quota.lastPDFTranslateService", usedService);
        ...
        async translateWithGPT(text, state = null) {
            ...
            if (!key)
                throw new Error("请先在设置中填写 GPT API Key。");
            ...
            if (protocol === "chat") {
                ...
                if (!value)
                    throw new Error(data?.error?.message || "GPT Chat Completions 返回为空");
            ...
            if (!value)
                throw new Error(data?.error?.message || "GPT Responses API 返回为空");
            this.recordTranslationUsage("gpt", asString(text).length, 1);
            return value.trim();
        },
        ...
        togglePrimaryTranslationView(state) {
            ...
            if (state.displayMode === "translation")
                this.setReaderMode(state, "source");
            else
                this.setReaderMode(state, "translation");
        },
        ...
        getPDFInnerDocument(state) {
            const view = this.getPDFView(state);
            try {
                return view?._iframeWindow?.document || view?._iframe?.contentWindow?.document || null;
            }
            catch (_) {
                return null;
            }
        ...
        injectPDFOverlayStyle(doc) {
            if (!doc || doc.getElementById("zft-pdf-overlay-style"))
                return;
            const style = doc.createElement("style");
      ...
      `;
        ...
        getLayoutTranslationGroups(state) {
            ...
            for (const seg of state.segments || []) {
                if (!Array.isArray(seg.bbox) || !seg.layoutBlockKey)
                    continue;
            ...
            for (const group of groups.values())
                group.translation = group.parts.filter(Boolean).join(" ").trim();
            return groups;
        },
        adaptGroupBBoxForProtectedLayout(group) {
            if (!group?.caption || !Array.isArray(group?.captionTargetBBox) || !Array.isArray(group?.bbox))
                return group?.bbox || null;
        ...
        getOriginalTextStyle(pageDiv, overlayNode) {
            try {
                ...
                for (const span of spans) {
                    ...
                    if (intersection / Math.min(targetArea, spanArea) < 0.08)
                        continue;
                    ...
                    if (!Number.isFinite(fontWeight))
                        fontWeight = /bold/i.test(cs.fontWeight) ? 700 : 400;
        ...
        applyOriginalTextStyle(pageDiv, node) {
            ...
            if (["paper-title", "author", "affiliation", "header", "footer"].includes(role))
                align = style.textAlign === "center" ? "center" : semantic.align;
            if (/caption$/.test(role))
                align = "left";
        ...
        styleAndFitOverlayPage(pageDiv, layer) {
            ...
            for (const node of layer.querySelectorAll(".zft-overlay-block"))
                this.applyOriginalTextStyle(pageDiv, node);
            this.fitOverlayPage(layer);
        },
        renderTranslationOverlay(state) {
            if (!state?.layoutAvailable || state.displayMode !== "translation")
                return;
            ...
            for (const group of groups.values()) {
                if (!group.translation)
                    continue;
                if (!byPage.has(group.pageIndex))
                    byPage.set(group.pageIndex, []);
            ...
            for (const [pageIndex, pageGroups] of byPage) {
                ...
                for (const group of pageGroups) {
                    ...
                    if (group.captionTargetType)
                        node.dataset.captionType = String(group.captionTargetType);
        ...
        observeOverlayPageSize(state, pageDiv, layer) {
            if (!pageDiv || !layer || typeof pageDiv.ownerDocument?.defaultView?.ResizeObserver !== "function")
                return;
            if (state.overlayResizeObservers.has(pageDiv))
                return;
        ...
        scheduleOverlayRender(state) {
            if (!state || state.overlayScheduled)
                return;
            state.overlayScheduled = setTimeout(() => {
                ...
                try {
                    ...
                    this.log("overlay render failed", String(e));
                }
            }, 120);
        },
        startOverlayLifecycle(state) {
            if (!state?.layoutAvailable)
                return;
        ...
        stopOverlayLifecycle(state) {
            ...
            if (state.overlayScheduled)
                clearTimeout(state.overlayScheduled);
            state.overlayScheduled = null;
            for (const observer of state.overlayResizeObservers?.values?.() || []) {
                try {
                    observer.disconnect();
        ...
        removeTranslationOverlays(state) {
            ...
            for (const doc of new Set(docs)) {
                try {
                    doc.querySelectorAll(".zft-page-translation-overlay").forEach((node) => node.remove());
                }
                catch (_) { }
            }
        },
        async exportNote(reader) {
            ...
            const state = itemID ? this.states.get(itemID) : null;
            if (!state?.completed)
                throw new Error("请先完成全文翻译，再导出 Zotero 笔记。");
            ...
            for (const seg of state.segments) {
                ...
                if (showSource)
                    blocks.push(`<p style="color:#666">${escapeHTML(seg.source)}</p>`);
        ...
        async exportSnapshot(reader) {
            ...
            const state = itemID ? this.states.get(itemID) : null;
            if (!state?.completed)
                throw new Error("请先完成全文翻译，再导出翻译快照。");
            const attachment = Zotero.Items.get(itemID);
            const parent = attachment.parentID ? Zotero.Items.get(attachment.parentID) : null;
        ...
        async exportLayoutPDF(reader, engine, options = {}) {
            const itemID = this.getAttachmentID(reader);
            if (!itemID)
                throw new Error("无法识别当前附件。");
            const state = this.getState(reader);
            if (state.running)
                throw new Error("当前已有翻译任务运行中，请先等待或取消当前任务。");
            ...
            try {
                ...
                const previousTranslationIDs = forceRetranslate
                    ? (await this.collectManagedTranslationAttachments(item)).map((x) => x.id)
                    : [];
                ...
                if (engine === "doc2x")
                    outPath = await this.runDoc2XPDF(item, path, temp, state);
                else if (engine === "zft-cloud") {
                    outPath = await this.runCloudPDF2ZH(item, path, temp, state, { forceRetranslate });
                    attachment = state.cloudResolvedAttachment || null;
                    state.cloudResolvedAttachment = null;
                }
                else
                    outPath = await this.runPDF2ZH(item, path, temp, state);
                this.throwIfCancelled(state);
                if (!attachment) {
                    if (!outPath)
                        throw new Error("翻译程序执行结束，但没有检测到输出 PDF。");
                    this.setStatus(state, "固定版式 PDF：正在导入 Zotero…", 95);
                    attachment = await this.attachGeneratedPDF(item, outPath, engine);
                }
                ...
                if (engine === "zft-cloud" && attachment?.id) {
                    this.cloudSetDocumentState(state, "SWITCHING", forceRetranslate ? "activate new translation version" : "bind translated attachment");
                    this.rememberComparePair(item, attachment);
                    this.cloudSetDocumentState(state, "BOUND", `attachment ${attachment.id}`);
                }
                ...
                if (forceRetranslate && attachment?.id && previousTranslationIDs.length) {
                    const removed = await this.removeSupersededTranslationAttachments(item, attachment, previousTranslationIDs);
                    if (removed)
                        this.log(`removed ${removed} superseded local translation attachment(s) after successful retranslation`);
                }
                return attachment;
            }
            catch (e) {
                if (e?.name === "ZFTCancelled") {
                    ...
                    if (engine === "zft-cloud")
                        this.cloudSetDocumentState(state, "ERROR", this.safeErrorMessage(e));
        ...
        resolveItemRef(itemRef) {
            ...
            if (!m)
                return null;
            try {
                return Zotero.Items.getByLibraryAndKey(Number(m[1]), m[2]) || Zotero.Items.get(Number(m[2])) || null;
            }
            catch (_) {
                return null;
            }
        ...
        cloudCompareOpenMap() {
            const value = this.cloudJSONPref("cloud.compareOpen", {});
            return value && typeof value === "object" && !Array.isArray(value) ? value : {};
        },
        rememberCompareOpen(sourceItem, translatedItem, open) {
            if (!sourceItem)
                return;
            const sourceRef = this.cloudClientItemRef(sourceItem);
            const map = this.cloudCompareOpenMap();
            if (open) {
                map[sourceRef] = {
                    sourceRef,
                    translatedRef: translatedItem ? this.cloudClientItemRef(translatedItem) : (map[sourceRef]?.translatedRef || ""),
                    updatedAt: Date.now(),
                };
            }
            else {
                delete map[sourceRef];
            }
            this.setCloudJSONPref("cloud.compareOpen", map);
        },
        isCompareRememberedOpen(sourceItem) {
            if (!sourceItem)
                return false;
            const sourceRef = this.cloudClientItemRef(sourceItem);
            return !!this.cloudCompareOpenMap()[sourceRef];
        },
        maintainNativeCompareSession(reader) {
            try {
                const state = this.states.get(this.getAttachmentID(reader));
                const session = state?.nativeCompare;
                if (!session)
                    return;
                const internal = this.readerInternal(reader);
                if (internal?.splitType !== "vertical") {
                    this.stopNativeCompareSession(state, { clearRemembered: true, disableSplit: false });
                }
            }
            catch (_) { }
        },
        scheduleNativeCompareRestore(reader, doc = null) {
            if (!reader)
                return;
            const state = this.getState(reader, doc || this.getReaderDoc(reader));
            if (state.nativeCompare || state.nativeCompareRestorePending || state.nativeCompareRestoreAttempted)
                return;
            const itemID = this.getAttachmentID(reader);
            ...
            const internal = this.readerInternal(reader);
            const persistedOpen = this.isCompareRememberedOpen(item);
            const legacyRestoredSplit = internal?.splitType === "vertical";
            if (!pair || pair.source.id !== item.id) {
                state.nativeCompareRestoreAttempted = true;
                return;
            }
            if (!persistedOpen && !legacyRestoredSplit) {
                if (!state.nativeCompareRestoreDeadline)
                    state.nativeCompareRestoreDeadline = Date.now() + 12000;
                if (Date.now() >= state.nativeCompareRestoreDeadline)
                    state.nativeCompareRestoreAttempted = true;
                return;
            }
            if (legacyRestoredSplit && !persistedOpen)
                this.rememberCompareOpen(pair.source, pair.translated, true);
            if (state.nativeCompareRestoreAttempts >= 6) {
                state.nativeCompareRestoreAttempted = true;
                return;
            }
            state.nativeCompareRestorePending = true;
            state.nativeCompareRestoreAttempts += 1;
            setTimeout(async () => {
                try {
                    try {
                    ...
                    if (!this.isLiveDocument(this.getReaderDoc(reader)))
                        throw new Error("reader document not ready");
                    if (state.nativeCompare) {
                        state.nativeCompareRestoreAttempted = true;
                        return;
                    }
                    await this.openNativeSplitComparison(reader, pair.source, pair.translated, { restoring: true });
                    state.nativeCompareRestoreAttempted = true;
                }
                catch (e) {
                    this.log("comparison restore retry", this.safeErrorMessage(e));
                    state.nativeCompareRestoreAttempted = false;
                }
                finally {
                    state.nativeCompareRestorePending = false;
        ...
        async openComparisonForReader(reader) {
            ...
            const pair = this.findComparePair(item);
            if (!pair)
                throw new Error("没有找到与当前 PDF 关联的原文/译文。");
            ...
            if (item.id !== pair.source.id) {
                ...
                try {
                    await sourceReader?._initPromise;
                }
                catch (_) { }
            }
        ...
        normalizeDOI(value) {
            let doi = asString(value || "").trim();
            if (!doi)
                return "";
            doi = doi.replace(/^https?:\/\/(?:dx\.)?doi\.org\//i, "").replace(/^doi\s*:\s*/i, "").trim();
            try {
                doi = decodeURIComponent(doi);
            }
            catch (_) { }
            doi = doi.replace(/[\s]+/g, "").replace(/[.,;:]+$/, "").toLowerCase();
            return /^10\.\d{4,9}\/\S+$/i.test(doi) ? doi : "";
        },
        documentDOIForAttachment(sourceItem) {
            if (!sourceItem)
                return "";
            const candidates = [];
            try {
                candidates.push(sourceItem.getField?.("DOI"));
            }
            catch (_) { }
            try {
                const parent = sourceItem.parentID ? Zotero.Items.get(sourceItem.parentID) : null;
                if (parent)
                    candidates.unshift(parent.getField?.("DOI"));
            }
            catch (_) { }
        ...
        cloudSetDocumentState(state, next, detail = "") {
            if (!state)
                return next;
            const current = asString(state.cloudDocumentState || "IDLE").toUpperCase();
            next = asString(next || "IDLE").toUpperCase();
            const allowed = {
                IDLE: ["RESOLVING", "UPLOADING", "RETRANSLATING", "ERROR"],
                RESOLVING: ["LOCAL_MATCH", "UPLOADING", "TRANSLATING", "DOWNLOADING", "BOUND", "IDLE", "ERROR"],
                LOCAL_MATCH: ["BOUND", "SWITCHING", "ERROR"],
                UPLOADING: ["TRANSLATING", "BOUND", "ERROR"],
                RETRANSLATING: ["UPLOADING", "TRANSLATING", "ERROR"],
                TRANSLATING: ["DOWNLOADING", "LOCAL_MATCH", "SWITCHING", "BOUND", "ERROR"],
                DOWNLOADING: ["SWITCHING", "BOUND", "ERROR"],
                SWITCHING: ["BOUND", "ERROR"],
                BOUND: ["RESOLVING", "RETRANSLATING", "SWITCHING", "IDLE", "ERROR"],
                ERROR: ["IDLE", "RESOLVING", "RETRANSLATING"],
            };
            if (current !== next && !(allowed[current] || []).includes(next)) {
                this.log(`cloud document state: ${current} -> ${next} (non-canonical transition)`);
            }
            state.cloudDocumentState = next;
            state.cloudDocumentStateDetail = asString(detail || "");
            this.log(`cloud document state: ${current} -> ${next}${detail ? ` · ${detail}` : ""}`);
            return next;
        },
        async cachedFileSHA256(path) {
            if (!path || !(await IOUtils.exists(path)))
                return "";
            const cache = this.cloudJSONPref("cloud.localFileHashIndex", {});
            let st = null;
            try {
                st = await IOUtils.stat(path);
            }
            catch (_) { }
            const size = Number(st?.size || 0);
            const mtime = Number(st?.lastModified || st?.lastModifiedTime || 0);
            const hit = cache[path];
            if (hit && Number(hit.size) === size && Number(hit.mtime) === mtime && /^[0-9a-f]{64}$/i.test(asString(hit.sha256))) {
                return asString(hit.sha256).toLowerCase();
            }
            const sha256 = (await this.sha256File(path)).toLowerCase();
            cache[path] = { sha256, size, mtime, checkedAt: Date.now() };
            const entries = Object.entries(cache).sort((a, b) => Number(b[1]?.checkedAt || 0) - Number(a[1]?.checkedAt || 0));
            this.setCloudJSONPref("cloud.localFileHashIndex", Object.fromEntries(entries.slice(0, 500)));
            return sha256;
        },
        async localPDFMatchesSHA256(path, expectedSha256) {
            ...
            if (!path || !(await IOUtils.exists(path)))
                return false;
            try {
                return (await this.cachedFileSHA256(path)) === expected;
            }
            catch (e) {
                this.log("local translated PDF hash check failed", path, this.safeErrorMessage(e));
                return false;
            }
        },
        async findReusableLocalTranslation(sourceItem, options = {}) {
            if (!sourceItem)
                return null;
            const expectedSha256 = asString(options.expectedSha256 || "").trim().toLowerCase();
            ...
            if (meta?.sourceRef === ref && (!meta.langOut || asString(meta.langOut) === targetLang)) {
                ...
                if (translated?.isAttachment?.()) {
                    try {
                        ...
                        if (path && await IOUtils.exists(path) && await this.localPDFMatchesSHA256(path, expectedSha256))
                            return translated;
                    }
                    catch (_) { }
                }
            ...
            try {
                ...
                for (const item of candidates.reverse()) {
                    ...
                    if (configuredTag) {
                        try {
                            tagged = (item.getTags?.() || []).some((x) => asString(x?.tag) === configuredTag);
                        }
                        catch (_) { }
                    }
                    if (!tagged && !/全文翻译|translated|translation/i.test(title))
                        continue;
                    ...
                    if (!path || !(await IOUtils.exists(path)))
                        continue;
                    if (!(await this.localPDFMatchesSHA256(path, expectedSha256)))
                        continue;
        ...
        async findSyncedLocalTranslationPath(sourceItem, expectedSha256 = "") {
            const expected = asString(expectedSha256 || "").trim().toLowerCase();
            if (!sourceItem || !expected)
                return null;
            try {
                const sourcePath = await sourceItem.getFilePathAsync();
                if (!sourcePath || !(await IOUtils.exists(sourcePath)))
                    return null;
                const parentDir = PathUtils.parent(sourcePath);
                const children = await IOUtils.getChildren(parentDir);
                const sourceName = basename(sourcePath).toLowerCase();
                const sourceStem = stem(sourcePath).toLowerCase();
                const likely = [];
                for (const path of children) {
                    if (path === sourcePath || !/\.pdf$/i.test(path))
                        continue;
                    let st;
                    try {
                        st = await IOUtils.stat(path);
                    ...
                    const looksTranslated = /translated|translation|全文翻译|译文|\.zh(?:-|_|\.)?cn|[-_.]mono/.test(name) ||
                        (sourceStem && name.startsWith(sourceStem));
                    if (!looksTranslated || name === sourceName)
                        continue;
                    likely.push({ path, mtime: Number(st?.lastModified || st?.lastModifiedTime || 0) });
                }
                likely.sort((a, b) => b.mtime - a.mtime);
                for (const candidate of likely) {
                    if (await this.localPDFMatchesSHA256(candidate.path, expected))
                        return candidate.path;
                }
            }
            catch (e) {
                this.log("synced local translation scan failed", this.safeErrorMessage(e));
            }
            return null;
        },
        async collectManagedTranslationAttachments(sourceItem) {
            if (!sourceItem)
                return [];
            const out = [];
            try {
                ...
                for (const id of ids) {
                    ...
                    if (!item?.id || item.id === sourceItem.id || !item.isAttachment?.())
                        continue;
                    const title = asString(item.getField?.("title") || "");
                    let tagged = false;
                    if (configuredTag) {
                    ...
                    if (!tagged && !/全文翻译\s*\(|ZFT Cloud|Zotero-full-translate/i.test(title))
                        continue;
                    const contentType = asString(item.attachmentContentType || item.getField?.("contentType") || "").toLowerCase();
                    if (contentType && !contentType.includes("pdf"))
                        continue;
        ...
        async removeSupersededTranslationAttachments(sourceItem, keepAttachment, previousIDs = []) {
            const keepID = Number(keepAttachment?.id) || 0;
            const ids = [...new Set((previousIDs || []).map(Number).filter((id) => id > 0 && id !== keepID))];
            let removed = 0;
            for (const id of ids) {
                const item = Zotero.Items.get(id);
                if (!item?.id || item.id === keepID || item.id === sourceItem?.id)
                    continue;
                try {
                    const pairs = this.cloudComparePairs();
                    const ref = this.cloudClientItemRef(item);
                ...
                try {
                    if (typeof item.eraseTx === "function")
                        await item.eraseTx();
                    else if (typeof Zotero.Items.eraseTx === "function")
                        await Zotero.Items.eraseTx([item.id]);
                    else if (typeof Zotero.Items.trashTx === "function")
                        await Zotero.Items.trashTx([item.id]);
                    removed++;
                }
                catch (e) {
                    this.log("failed to remove superseded translation", id, this.safeErrorMessage(e));
                }
            }
            return removed;
        },
        ...
        stopNativeCompareSession(state, options = {}) {
            const session = state?.nativeCompare;
            if (!session)
                return;
            const clearRemembered = !!options.clearRemembered;
            const disableSplit = options.disableSplit !== false;
            try {
                session.primaryContainer?.removeEventListener("scroll", session.onPrimaryScroll);
            }
            catch (_) { }
            try {
                session.secondaryContainer?.removeEventListener("scroll", session.onSecondaryScroll);
            }
            catch (_) { }
            ...
            for (const binding of session.eventBusBindings || []) {
                try {
                    if (typeof binding.bus?.off === "function")
                        binding.bus.off(binding.type, binding.handler);
                    else if (typeof binding.bus?._off === "function")
                        binding.bus._off(binding.type, binding.handler);
                }
                catch (_) { }
            }
            ...
            try {
                ...
                if (session.syncRaf && win?.cancelAnimationFrame)
                    win.cancelAnimationFrame(session.syncRaf);
            }
            catch (_) { }
            session.syncRaf = 0;
            try {
            ...
            if (disableSplit && session.restoreSplit !== false) {
                try {
                    session.internal?.disableSplitView?.();
                }
                catch (_) { }
            }
            if (clearRemembered) {
                try {
                    const source = session.sourceItemID ? Zotero.Items.get(session.sourceItemID) : null;
                    this.rememberCompareOpen(source, null, false);
                }
                catch (_) { }
            }
            state.nativeCompare = null;
        ...
        bindNativeCompareScroll(state, session) {
            ...
            const markActive = (side) => {
                ...
                try {
                    session.internal._lastViewPrimary = side === "primary";
                }
                catch (_) { }
            };
            const scheduleSync = (side, pageNumber = null, force = false) => {
                if (!enabled())
                    return;
                if (!force && session.activeSide !== side)
                    return;
                ...
                session.syncRaf = request(() => {
                    ...
                    session.pendingSyncSide = null;
                    session.pendingPageNumber = null;
                    if (!enabled() || session.activeSide !== fromSide)
                        return;
            ...
            for (const side of ["primary", "secondary"]) {
                ...
                if (bus) {
                    const pageHandler = () => {
                        ...
                        if (lastViewSide === side)
                            markActive(side);
                        scheduleSync(side);
                    };
                    try {
                        if (typeof bus.on === "function")
                            bus.on("pagechanging", pageHandler);
                        else if (typeof bus._on === "function")
                            bus._on("pagechanging", pageHandler);
        ...
        ensureNativeCompareBadge(state, session) {
            ...
            if (!doc)
                return null;
            try {
                doc.getElementById(COMPARE_BADGE_ID)?.remove();
            }
            catch (_) { }
            ...
            close.textContent = "×";
            close.title = "关闭译文对照";
            close.addEventListener("click", () => this.stopNativeCompareSession(state, { clearRemembered: true }));
            badge.append(label, retranslate, close);
            doc.body.append(badge);
            session.badge = badge;
            return badge;
        },
        async openNativeSplitComparison(reader, sourceItem, translatedItem, options = {}) {
            if (!reader || !sourceItem?.id || !translatedItem?.id)
                throw new Error("原文或译文附件不可用。");
            const state = this.getState(reader, this.getReaderDoc(reader));
            this.stopNativeCompareSession(state);
            ...
            try {
                internal._secondaryView?.destroy?.();
            }
            catch (_) { }
            try {
                internal._secondaryViewContainer?.replaceChildren?.();
            }
            catch (_) { }
            ...
            this.rememberCompareOpen(sourceItem, translatedItem, true);
            ...
            if (initial)
                this.applyPDFViewScrollSnapshot(secondaryView, initial);
            return session;
        },
        ...
        cloudClientID() {
            ...
            if (id)
                return id;
            try {
                id = Services.uuid.generateUUID().toString().replace(/[{}]/g, "");
            }
            catch (_) {
                id = `zotero-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 12)}`;
            }
        ...
        cloudProviderPoolSnapshot() {
            const value = this.cloudJSONPref("cloud.providerPoolCache", {});
            const items = Array.isArray(value?.items) ? value.items : [];
            const defaults = Array.isArray(value?.default_provider_ids) ? value.default_provider_ids : [];
            return {
                items,
                default_provider_ids: defaults,
                default_provider_strategy: asString(value?.default_provider_strategy || "balanced"),
                updated_at: Number(value?.updated_at || 0),
            };
        },
        ensureCloudProviderPoolTimer() {
            if (this.cloudProviderPoolTimer)
                return;
            if (!asString(this.pref("cloud.baseURL", "")).trim() || !asString(this.pref("cloud.apiKey", "")).trim())
                return;
            this.cloudProviderPoolTimer = setInterval(() => {
                this.refreshCloudProviderPool(true).catch(() => {});
            }, 60000);
        },
        async refreshCloudProviderPool(force = false) {
            this.ensureCloudProviderPoolTimer();
            const cached = this.cloudProviderPoolSnapshot();
            if (!force && cached.items.length && Date.now() - cached.updated_at < 30000)
                return cached;
            const pool = await this.cloudPDF2ZHJSON("GET", "/api/v1/account/provider-pool", null);
            const normalized = {
                items: Array.isArray(pool?.items) ? pool.items.map((x) => ({
                    id: asString(x?.id || "").trim(),
                    kind: asString(x?.kind || "").trim(),
                    display_name: asString(x?.display_name || x?.kind || x?.id || "API").trim(),
                    vendor: asString(x?.vendor || x?.kind || "").trim(),
                    template_id: asString(x?.template_id || "").trim(),
                    qps: Number(x?.qps || 0),
                    max_concurrency: Number(x?.max_concurrency || 0),
                    quota_status: asString(x?.quota_status || "normal"),
                    quota_enabled: x?.quota_enabled !== false,
                    quota_period: asString(x?.quota_period || "month"),
                    quota_total_chars: x?.quota_total_chars == null ? null : Number(x.quota_total_chars),
                    quota_used_chars: x?.quota_used_chars == null ? null : Number(x.quota_used_chars),
                    quota_remaining_chars: x?.quota_remaining_chars == null ? null : Number(x.quota_remaining_chars),
                    quota_remaining_percent: x?.quota_remaining_percent == null ? null : Number(x.quota_remaining_percent),
                    quota_reserve_chars: Number(x?.quota_reserve_chars || 0),
                    quota_low_percent: Number(x?.quota_low_percent ?? 10),
                    quota_reset_at: asString(x?.quota_reset_at || ""),
                    last_test_ok: x?.last_test_ok === true ? true : x?.last_test_ok === false ? false : null,
                    selected_by_default: !!x?.selected_by_default,
                })).filter((x) => x.id) : [],
                default_provider_ids: Array.isArray(pool?.default_provider_ids) ? pool.default_provider_ids.map((x) => asString(x).trim()).filter(Boolean) : [],
                default_provider_strategy: asString(pool?.default_provider_strategy || "balanced").toLowerCase(),
                updated_at: Date.now(),
            };
            this.setCloudJSONPref("cloud.providerPoolCache", normalized);
            this.cloudProviderPoolUpdatedAt = normalized.updated_at;
            this.refreshToolbarQuotaWarnings();
            return normalized;
        },
        cloudProviderSelection() {
            const mode = asString(this.pref("cloud.providerMode", "server-default"));
            if (mode !== "custom")
                return { mode: "server-default", providers: [], strategy: "balanced" };
            const raw = this.cloudJSONPref("cloud.providerIDs", []);
            const ids = Array.isArray(raw) ? [...new Set(raw.map((x) => asString(x).trim()).filter(Boolean))] : [];
        ...
        async resolveCloudProviderSelection() {
            const selection = this.cloudProviderSelection();
            if (selection.mode !== "custom")
                return selection;
            const pool = await this.refreshCloudProviderPool(true);
            const available = new Set((pool.items || []).map((x) => x.id));
            const providers = selection.providers.filter((id) => available.has(id));
            if (!providers.length)
                throw new Error("请在设置中选择至少一个当前可用的 Cloud API 实例。");
            if (providers.length !== selection.providers.length)
                this.setCloudJSONPref("cloud.providerIDs", providers);
            return { ...selection, providers, strategy: providers.length === 1 ? "single" : selection.strategy };
        },
        cloudProviderSummary() {
            const pool = this.cloudProviderPoolSnapshot();
            const count = pool.items.length;
            if (!count)
                return "API 池未同步";
            const selection = this.cloudProviderSelection();
            if (selection.mode === "server-default") {
                const defaults = pool.default_provider_ids.length || count;
                const strategy = pool.default_provider_strategy === "failover" ? "主备切换" : defaults <= 1 ? "单实例" : "负载均衡";
                return `API 池 ${defaults} 个实例 · ${strategy}`;
            }
            const available = new Set(pool.items.map((x) => x.id));
            const selected = selection.providers.filter((id) => available.has(id)).length;
            const strategy = selected <= 1 ? "单实例" : selection.strategy === "failover" ? "主备切换" : "负载均衡";
            return `API 池 ${selected}/${count} 个实例 · ${strategy}`;
        },
        formatCloudQuotaChars(value) {
        ...
        activeCloudProviderItems(pool = null) {
            pool = pool || this.cloudProviderPoolSnapshot();
            const items = Array.isArray(pool?.items) ? pool.items : [];
            const selection = this.cloudProviderSelection();
            let ids = [];
            if (selection.mode === "custom")
                ids = selection.providers;
            else
                ids = Array.isArray(pool?.default_provider_ids) && pool.default_provider_ids.length ? pool.default_provider_ids : items.map((x) => x.id);
            const wanted = new Set(ids.map((x) => asString(x).trim()).filter(Boolean));
            return items.filter((x) => wanted.has(asString(x?.id).trim()));
        },
        cloudQuotaWarnings(pool = null) {
            const items = this.activeCloudProviderItems(pool);
            return items.filter((item) => {
                ...
                if (!Number.isFinite(total) || total <= 0 || !Number.isFinite(remaining))
                    return false;
                const status = asString(item?.quota_status || "").toLowerCase();
                if (["low", "exhausted", "unavailable"].includes(status))
                    return true;
                ...
                return Number.isFinite(pct) && Number.isFinite(low) && pct <= low;
            });
        },
        cloudQuotaWarningLevel(pool = null) {
            const warnings = this.cloudQuotaWarnings(pool);
            if (warnings.some((x) => ["exhausted", "unavailable"].includes(asString(x?.quota_status).toLowerCase()) || Number(x?.quota_remaining_chars) <= Number(x?.quota_reserve_chars || 0)))
                return "critical";
            return warnings.length ? "low" : "";
        },
        cloudQuotaWarningText(pool = null) {
            const warnings = this.cloudQuotaWarnings(pool);
            if (!warnings.length)
                return "";
            const parts = warnings.slice(0, 3).map((item) => {
                const status = asString(item?.quota_status).toLowerCase();
                const name = asString(item?.display_name || item?.kind || "API");
                if (["exhausted", "unavailable"].includes(status) || Number(item?.quota_remaining_chars) <= Number(item?.quota_reserve_chars || 0))
                    return `${name} 已无可用额度`;
                const pct = item?.quota_remaining_percent == null ? NaN : Number(item.quota_remaining_percent);
                const pctText = Number.isFinite(pct) ? `（${pct.toFixed(pct < 10 ? 1 : 0)}%）` : "";
                return `${name} 剩余 ${this.formatCloudQuotaChars(item?.quota_remaining_chars)}${pctText}`;
            });
            if (warnings.length > 3)
                parts.push(`另 ${warnings.length - 3} 个`);
            return parts.join(" · ");
        },
        refreshToolbarQuotaWarnings() {
            for (const state of this.states.values()) {
                try {
                    this.updateToolbarTaskState(state);
                }
        ...
        openCloudConsole() {
            ...
            if (!base)
                throw new Error("请先填写 Zotero Full Translate Cloud 地址。");
            try {
                Zotero.launchURL(base);
            }
            catch (_) {
                try {
                    Services.externalProtocolService.loadURI(Services.io.newURI(base));
                }
                catch (_) { }
            }
        },
        ...
        getOrCreateCloudRequestID(itemRef) {
            ...
            if (tokens[itemRef])
                return asString(tokens[itemRef]);
            let requestID;
            try {
                requestID = Services.uuid.generateUUID().toString().replace(/[{}]/g, "");
            }
            catch (_) {
                requestID = `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 14)}`;
            }
        ...
        async syncCloudTranslationForReader(reader) {
            if (!this.pref("cloud.autoSyncTranslations", true) || !this.cloudAuthToken())
                return null;
            const itemID = this.getAttachmentID(reader);
            const item = itemID ? Zotero.Items.get(itemID) : null;
            if (!item?.isAttachment?.())
                return null;
            const title = asString(item.getField?.("title") || "");
            if (/全文翻译|translated|translation/i.test(title))
                return null;
            const itemRef = this.cloudClientItemRef(item);
            if (this.cloudAutoSynced.has(itemRef) || this.cloudAutoSyncing.has(itemRef))
                return null;
            const state = this.getState(reader);
            this.cloudAutoSyncing.add(itemRef);
            try {
                const path = await item.getFilePathAsync();
                if (!path || !(await IOUtils.exists(path)))
                    return null;
                const doi = this.documentDOIForAttachment(item);
                this.cloudSetDocumentState(state, "RESOLVING", doi ? `DOI ${doi}` : "DOI unavailable");
                if (!doi) {
                    const fallback = await this.findReusableLocalTranslation(item);
                    if (fallback?.id) {
                        this.cloudSetDocumentState(state, "LOCAL_MATCH", "local translation without DOI");
                        this.rememberComparePair(item, fallback);
                        this.cloudSetDocumentState(state, "BOUND", "local fallback");
                        return fallback;
                    }
                    this.cloudSetDocumentState(state, "IDLE", "no DOI");
                    return null;
                }
                const requestID = this.getOrCreateCloudRequestID(itemRef);
                const lookup = await this.cloudPDF2ZHJSON("POST", "/api/v1/jobs/lookup", null, {
                    document_doi: doi,
                    lang_in: this.toPDF2ZHLang(this.pref("sourceLanguage", "auto")),
                    lang_out: this.toPDF2ZHLang(this.pref("targetLanguage", "zh-CN")),
                    pages: null,
                    output_mode: "mono",
                    filename: basename(path),
                    client_id: this.cloudClientID(),
                    client_request_id: requestID,
                    client_item_key: itemRef,
                });
                if (!lookup?.found || !lookup?.job?.id) {
                    const fallback = await this.findReusableLocalTranslation(item);
                    if (fallback?.id) {
                        this.cloudSetDocumentState(state, "LOCAL_MATCH", "local fallback");
                        this.rememberComparePair(item, fallback);
                        this.cloudAutoSynced.add(itemRef);
                        this.cloudSetDocumentState(state, "BOUND", "local fallback");
                    }
                    else {
                        this.cloudSetDocumentState(state, "IDLE", "DOI not bound");
                    }
                    return fallback || null;
                }
                this.rememberCloudJob(lookup.job, item, requestID);
                const attachment = await this.importCompletedCloudJob(lookup.job, { notify: false });
                if (attachment?.id) {
                    this.rememberComparePair(item, attachment);
                    this.cloudAutoSynced.add(itemRef);
                    this.cloudSetDocumentState(state, attachment?._zftReusedLocal ? "LOCAL_MATCH" : "SWITCHING", "resolved current DOI translation");
                    this.cloudSetDocumentState(state, "BOUND", `job ${lookup.job.id}`);
                    const reusedLocal = !!lookup.job?.local_reuse || !!attachment?._zftReusedLocal;
                    this.notify("Zotero Full Translate", reusedLocal ? "检测到本机已有当前译文版本，已直接绑定，不重复下载。" : "检测到此 DOI 已有账户译文，已自动绑定原文/译文。");
                ...
                this.cloudAutoSyncing.delete(itemRef);
            }
        },
        cloudStageLabel(stage) {
            ...
            if (labels[value])
                return labels[value];
            if (/retrying babeldoc/.test(value))
                return "重试翻译任务";
            if (/layout/.test(value))
                return "页面布局分析";
            if (/paragraph/.test(value))
                return "段落分析";
            if (/style|formula/.test(value))
                return "样式与公式分析";
            if (/font/.test(value))
                return "字体映射";
            if (/translate/.test(value))
                return "多引擎翻译";
            if (/save|pdf|render|create/.test(value))
                return "生成固定版式 PDF";
            return asString(stage || "处理中");
        },
        async importCompletedCloudJob(job, { notify = true } = {}) {
            const jobID = asString(job?.id || "").trim();
            if (!jobID || this.isCloudJobImported(jobID) || this.cloudImporting.has(jobID))
                return null;
            ...
            try {
                ...
                if (!item?.isAttachment?.()) {
                    if (notify)
                        this.notify("Zotero Full Translate", `任务 ${jobID.slice(0, 8)} 已完成，但找不到原 Zotero 附件，结果保留在 Cloud 用户中心。`);
                    return null;
                }
                const expectedSha256 = asString(job?.mono_sha256 || "").trim().toLowerCase();
                let attachment = expectedSha256
                    ? await this.findReusableLocalTranslation(item, { expectedSha256 })
                    : null;
                if (attachment?.id) {
                    try {
                        attachment._zftReusedLocal = true;
                    }
                    catch (_) { }
                    this.rememberComparePair(item, attachment);
                    this.markCloudJobImported(jobID, itemRef);
                    if (notify)
                        this.notify("Zotero Full Translate", `${job.filename || "PDF"} 的当前译文已在本机，跳过下载并直接绑定。`);
                    return attachment;
                }
                const syncedPath = await this.findSyncedLocalTranslationPath(item, expectedSha256);
                if (syncedPath) {
                    attachment = await this.attachGeneratedPDF(item, syncedPath, "zft-cloud");
                    if (attachment?.id) {
                        try {
                            attachment._zftReusedLocal = true;
                        }
                        ...
                        this.markCloudJobImported(jobID, itemRef);
                        if (notify)
                            this.notify("Zotero Full Translate", `${job.filename || "PDF"} 已由本地同步软件提供译文，已直接绑定。`);
                        return attachment;
                    }
                }
                ...
                if (expectedSha256 && !(await this.localPDFMatchesSHA256(outPath, expectedSha256))) {
                    throw new Error("Cloud 下载译文的 SHA-256 与任务记录不一致，已拒绝导入。");
                }
                attachment = await this.attachGeneratedPDF(item, outPath, "zft-cloud");
                if (attachment?.id)
                    this.rememberComparePair(item, attachment);
                this.markCloudJobImported(jobID, itemRef);
                if (notify)
                    this.notify("Zotero Full Translate", `${job.filename || "PDF"} 云端翻译完成，已自动添加到 Zotero。`);
                return attachment;
            }
            finally {
        ...
        async monitorRecoveredCloudJob(initialJob) {
            ...
            if (!jobID || this.cloudMonitors.has(jobID))
                return;
                ...
                while (true) {
                    ...
                    if (status === "COMPLETED") {
                        if (this.pref("cloud.autoImportCompleted", true))
                            await this.importCompletedCloudJob(job, { notify: true });
                        return;
                    }
        ...
        async restoreCloudJobs({ notify = false } = {}) {
            if (!this.pref("cloud.autoRestore", true) && !notify)
                return { total: 0, active: 0 };
            const base = this.cloudPDF2ZHBaseURL();
            if (!base || !this.cloudHasCredentials())
                return { total: 0, active: 0 };
            ...
            if (notify)
                this.notify("Zotero Full Translate", `已检查 ${jobs.length} 个云端任务：${active} 个进行中，${completed} 个待导入。`);
        ...
        cloudAuthToken() {
            return asString(this.pref("cloud.apiKey", this.pref("pdf2zh.cloud.apiKey", ""))).trim();
        },
        cloudHasCredentials() {
            return !!this.cloudAuthToken();
        },
        cloudHandleAuthFailure(status, detail = "") {
            if (Number(status) !== 401 || !this.cloudAuthToken())
                return false;
            this.setCloudJSONPref("cloud.accountInfo", {});
            this.cloudAutoSynced.clear();
            this.cloudAutoSyncing.clear();
            const suffix = asString(detail).trim();
            const error = new Error(`Cloud API Key 无效或已被撤销${suffix ? `：${suffix}` : "。请在 3005 用户中心创建或检查 API Key。"}`);
            error.name = "ZFTCloudApiKeyInvalid";
            throw error;
        },
        cloudPDF2ZHHeaders() {
            const key = this.cloudAuthToken();
            if (!key)
                return {};
            let platform = "Zotero";
            try {
                platform = `${Services.appinfo.OS || "Desktop"} · Zotero ${Zotero.version || ""}`;
            }
            catch (_) { }
            return {
                "Authorization": `Bearer ${key}`,
                "X-ZFT-Device-ID": this.cloudClientID(),
                "X-ZFT-Platform": platform,
                "X-ZFT-Client-Version": this.version,
            };
        },
        cloudAccountInfo() {
            return this.cloudJSONPref("cloud.accountInfo", {});
        },
        async cloudVerifyApiKey() {
            const base = this.cloudPDF2ZHBaseURL();
            const key = this.cloudAuthToken();
            if (!base)
                throw new Error("请先填写 Zotero Full Translate Cloud 服务器地址。");
            if (!key)
                throw new Error("请填写在 Cloud 3005 用户中心创建的账户 API Key。");
            const result = await this.testCloudPDF2ZH();
            if (!result?.account?.user)
                throw new Error("Cloud API Key 未能解析到账户。");
            this.setCloudJSONPref("cloud.accountInfo", { user: result.account.user || {}, verified_at: Date.now() });
            this.setPref("cloud.authToken", "");
            return result;
        },
        async cloudLogout() {
            this.setPref("cloud.apiKey", "");
            this.setPref("pdf2zh.cloud.apiKey", "");
            this.setPref("cloud.authToken", "");
            this.setCloudJSONPref("cloud.accountInfo", {});
            this.setCloudJSONPref("cloud.providerPoolCache", {});
            if (this.cloudProviderPoolTimer)
                clearInterval(this.cloudProviderPoolTimer);
            this.cloudProviderPoolTimer = null;
            this.cloudAutoSynced.clear();
        ...
        async testCloudPDF2ZH() {
            ...
            try {
                ...
                throw new Error(`无法连接 Zotero Full Translate Cloud：${this.safeErrorMessage(e)}`);
            }
            const hs = Number(healthRes?.status) || 0;
            const health = healthRes?.response && typeof healthRes.response === "object" ? healthRes.response : (() => { try {
                return JSON.parse(healthRes?.responseText || "{}");
            }
            catch (_) {
                return {};
            } })();
            if (hs < 200 || hs >= 300 || !health?.ok)
                throw new Error(`Cloud 健康检查失败：HTTP ${hs || "?"}`);
            if (!this.cloudHasCredentials())
                throw new Error("服务器可访问，但尚未配置账户 API Key。");
            let identity = null;
            try {
                identity = await this.cloudPDF2ZHJSON("GET", "/api/v1/auth/client", null);
            }
            catch (e) {
                throw new Error(`Cloud API Key 验证失败：${this.safeErrorMessage(e)}`);
            }
            const account = {
                user: identity?.user || null,
                api_key_id: identity?.api_key_id || null,
                key_prefix: identity?.key_prefix || "",
                scopes: identity?.scopes || [],
                expires_at: identity?.expires_at || null,
                device: identity?.device || null,
            };
            return { ...health, account, elapsedMs: Date.now() - started };
        },
        async uploadCloudPDF2ZHJob(item, path, filename, state = null, options = {}) {
            const base = this.cloudPDF2ZHBaseURL();
            if (!base)
                throw new Error("请在设置中填写 Zotero Full Translate Cloud 地址。");
            if (!this.cloudHasCredentials())
                throw new Error("请先配置并验证 Cloud 账户 API Key。");
            ...
            const itemRef = this.cloudClientItemRef(item);
            if (forceRetranslate)
                this.clearCloudRequestID(itemRef);
            const requestID = this.getOrCreateCloudRequestID(itemRef);
            const documentDOI = this.documentDOIForAttachment(item);
            if (state)
                this.cloudSetDocumentState(state, forceRetranslate ? "RETRANSLATING" : "RESOLVING", documentDOI ? `DOI ${documentDOI}` : "DOI unavailable");
            if (this.pref("cloud.reuseHistory", true) && !forceRetranslate && documentDOI) {
                if (state)
                    this.setStatus(state, "Cloud：按 DOI 检查账户译文…", 7);
                const lookup = await this.cloudPDF2ZHJSON("POST", "/api/v1/jobs/lookup", state, {
                    document_doi: documentDOI, lang_in: langIn, lang_out: langOut, pages: null, output_mode: "mono",
                    filename: filename || "document.pdf", client_id: clientID, client_request_id: requestID, client_item_key: itemRef,
                });
                if (lookup?.found && lookup?.job?.id) {
                    this.rememberCloudJob(lookup.job, item, requestID);
                    if (state) {
                        state.cloudHistoryHit = true;
                        this.cloudSetDocumentState(state, "TRANSLATING", "translation already completed in Cloud");
                        this.setStatus(state, "Cloud：命中 DOI 译文 · 跳过上传和翻译", 90);
                    }
                    return lookup.job;
                }
            }
            const selection = await this.resolveCloudProviderSelection();
            const bytes = await IOUtils.read(path);
            return new Promise((resolve, reject) => {
                ...
                try {
                    ...
                    for (const [header, value] of Object.entries(this.cloudPDF2ZHHeaders()))
                    ...
                    if (state)
                        this.cloudSetDocumentState(state, "UPLOADING", documentDOI ? `DOI ${documentDOI}` : "no DOI; translation only");
                    ...
                    if (documentDOI)
                        form.append("document_doi", documentDOI);
                    if (forceRetranslate)
                        form.append("force_retranslate", "true");
                    form.append("client_id", clientID);
                    form.append("client_request_id", requestID);
                    form.append("client_item_key", itemRef);
                    if (selection.mode === "custom") {
                        if (!selection.providers.length)
                            throw new Error("自定义云端引擎模式下至少选择一个翻译引擎。");
                        form.append("providers", selection.providers.join(","));
                        form.append("provider_strategy", selection.strategy);
                    }
                    if (state)
                        state.currentAbort = () => { try {
                            xhr.abort();
                        }
                        catch (_) { } };
                    xhr.upload.onprogress = (event) => {
                        if (!state || !event.lengthComputable)
                            return;
                        const pct = Math.round((event.loaded / event.total) * 100);
                        this.setStatus(state, `Cloud：上传 PDF ${pct}%`, Math.min(18, 5 + Math.round(pct * 0.13)));
                    };
                    const fail = (message) => {
                    ...
                    xhr.onload = () => {
                        ...
                        if (status >= 200 && status < 300 && data?.id) {
                            ...
                            try {
                                this.cloudHandleAuthFailure(status, data?.detail || raw);
                            }
                            catch (e) {
                                reject(e);
                                return;
                            }
                            reject(new Error(`Cloud 任务创建失败：HTTP ${status || "?"} ${asString(data?.detail || raw).slice(0, 500)}`));
                        }
                    };
                    xhr.onerror = () => fail("Cloud 上传网络错误");
                    xhr.ontimeout = () => fail("Cloud 上传超时");
                    xhr.onabort = () => fail("Cloud 上传已取消");
                    xhr.send(form);
                }
                catch (e) {
                    reject(new Error(`无法创建 Cloud 上传请求：${this.safeErrorMessage(e)}`));
                }
            });
        },
        async cloudPDF2ZHJSON(method, path, state = null, body = null) {
            ...
            try {
                ...
                const data = res?.response && typeof res.response === "object" ? res.response : (() => { try {
                ...
                if (status < 200 || status >= 300) {
                    this.cloudHandleAuthFailure(status, data?.detail || data?.error_message || "");
                    throw new Error(`HTTP ${status || "?"} ${asString(data?.detail || data?.error_message || "")}`.trim());
                }
                return data;
            }
            finally {
                if (state?.currentAbort === aborter)
                    state.currentAbort = null;
            }
        },
        ...
        async downloadCloudPDF2ZHResult(jobID, outPath, state = null) {
            ...
            try {
                ...
                const status = Number(res?.status) || 0;
                if (status < 200 || status >= 300) {
                    this.cloudHandleAuthFailure(status, "下载译文时鉴权失败");
                    throw new Error(`下载固定版式 PDF 失败：HTTP ${status || "?"}`);
                }
                const buf = res?.response;
                if (!buf)
                    throw new Error("Cloud 返回的 PDF 内容为空。");
        ...
        async runCloudPDF2ZH(item, path, outDir, state = null, options = {}) {
            await this.testCloudPDF2ZH();
            if (state)
                this.setStatus(state, "Cloud：连接正常", 5);
            this.throwIfCancelled(state);
            const created = await this.uploadCloudPDF2ZHJob(item, path, basename(path), state, options);
            const jobID = asString(created?.id || "").trim();
            if (!jobID)
                throw new Error("Cloud 没有返回任务 ID。");
            if (asString(created?.status || "").toUpperCase() === "COMPLETED") {
                const expectedSha256 = asString(created?.mono_sha256 || "").trim().toLowerCase();
                let localAttachment = expectedSha256
                    ? await this.findReusableLocalTranslation(item, { expectedSha256 })
                    : null;
                if (!localAttachment && expectedSha256) {
                    const syncedPath = await this.findSyncedLocalTranslationPath(item, expectedSha256);
                    if (syncedPath)
                        localAttachment = await this.attachGeneratedPDF(item, syncedPath, "zft-cloud");
                }
                if (localAttachment?.id) {
                    if (state)
                        this.cloudSetDocumentState(state, "LOCAL_MATCH", "current translated PDF already local");
                    try {
                        localAttachment._zftReusedLocal = true;
                    }
                    catch (_) { }
                    this.rememberComparePair(item, localAttachment);
                    this.markCloudJobImported(jobID, this.cloudClientItemRef(item));
                    if (state) {
                        state.cloudResolvedAttachment = localAttachment;
                        state.cloudHistoryHit = true;
                        state.lastCompletedCloudJobID = null;
                        state.remoteLayoutJobID = null;
                        this.setStatus(state, "Cloud：本机已有当前译文版本 · 跳过下载", 94);
                    }
                    if (state)
                        this.cloudSetDocumentState(state, "BOUND", `job ${jobID}`);
                    return null;
                }
            }
            if (state) {
                state.remoteLayoutJobID = jobID;
                this.cloudSetDocumentState(state, "TRANSLATING", `job ${jobID}`);
            }
            const pollMs = Math.max(500, Number(this.pref("cloud.pollMs", 1200)) || 1200);
            let completedJob = created;
            try {
                while (true) {
                    this.throwIfCancelled(state);
                    ...
                    const stageCurrent = Number(job?.metrics?.stage_current);
                    const stageTotal = Number(job?.metrics?.stage_total);
                    const stageDetail = Number.isFinite(stageCurrent) && Number.isFinite(stageTotal) && stageTotal > 0
                        ? ` · 子任务 ${Math.round(stageCurrent)}/${Math.round(stageTotal)}`
                        : stageProgress > 0 ? ` · 阶段 ${Math.round(stageProgress)}%` : "";
                    if (state)
                        this.setStatus(state, `Cloud：${this.cloudStageLabel(job?.stage || status)}${stageDetail}${engines ? ` · ${engines}` : ""} · 总进度 ${Math.round(remoteProgress)}%`, Math.min(92, localProgress));
                    if (status === "COMPLETED") {
                        completedJob = job;
                        break;
                    }
                    if (status === "CANCELLED") {
                        const e = new Error("Cloud 翻译任务已取消");
                        e.name = "ZFTCancelled";
                        throw e;
                    }
                    if (status === "FAILED") {
                        ...
                        if (/asyncio\.exceptions\.CancelledError|BabelDOC internal CancelledError/i.test(remoteError)) {
                            throw new Error("Cloud：翻译任务内部中断；服务器会自动重试一次。若仍失败，请重新提交任务。");
                        }
                        throw new Error(`Cloud 失败：${remoteError}`);
                    }
                    await Zotero.Promise.delay(pollMs);
                }
                this.throwIfCancelled(state);
                if (state) {
                    this.cloudSetDocumentState(state, "DOWNLOADING", `job ${jobID}`);
                    this.setStatus(state, "Cloud：下载译文 PDF…", 94);
                }
                ...
                const expectedSha256 = asString(completedJob?.mono_sha256 || "").trim().toLowerCase();
                if (expectedSha256 && !(await this.localPDFMatchesSHA256(outPath, expectedSha256))) {
                    throw new Error("Cloud 下载译文的 SHA-256 与任务记录不一致，已拒绝导入。");
                }
                if (state) {
                ...
                if (state?.cancelRequested) {
                    try {
                        await this.cancelCloudPDF2ZHJob(state);
                    }
                    catch (_) { }
                    const cancelled = new Error("Cloud 翻译任务已取消");
        ...
        async runPDF2ZH(item, path, outDir, state = null) {
            ...
            if (serviceArg)
                args.push(...splitArgs(serviceArg));
            ...
            if (rate.enabled) {
                ...
                if (state)
                    this.setStatus(state, `pdf2zh_next：固定版式翻译 · ${pdfQPS} QPS · ${workers} workers`, 15);
            }
        ...
        async runDoc2XPDF(item, path, outDir, state = null) {
            ...
            if (termID)
                args.push("--term-id", termID);
            if (this.pref("doc2x.contextual", true))
                args.push("--contextual-translation");
            if (this.pref("ignoreReferences", true))
                args.push("--ignore-translate-types", "reference");
        ...
        startPageSync(state) {
            if (!this.pref("syncReaderPage", true) || state.syncTimer)
                return;
            let last = -1;
        ...
        async loadTranslationCache(state) {
            if (!this.pref("cache.enabled", true) || !state.cacheKey)
                return null;
            try {
        ...
        async putPresignedFileWithoutContentType(url, path, timeout = 180000, state = null) {
            ...
            return new Promise((resolve, reject) => {
                ...
                xhr.onloadend = () => {
                    ...
                    try {
                        responseText = asString(xhr.responseText || xhr.response || "");
                    }
                    catch (_) { }
                ...
                xhr.onerror = () => {
                    ...
                    try {
                        responseText = asString(xhr.responseText || "");
                    }
                    catch (_) { }
        ...
        safeErrorMessage(error) {
            ...
            try {
                return asString(error, "未知错误");
            }
            catch (_) {
                return "无法读取错误对象（dead object）";
            }
        },
        snapshotNetworkError(error) {
            ...
            if (xhr) {
                if (!snap.status)
                    snap.status = Number(this.safeRead(xhr, "status", 0)) || 0;
                if (!snap.responseText)
                    snap.responseText = asString(this.safeRead(xhr, "responseText", ""));
            }
            return snap;
        },
        ...
        async extractFullMarkdownFromZip(zipPath, outDir) {
            ...
            try {
                ...
                if (!entry)
                    throw new Error("MinerU ZIP 中未找到 full.md。");
        ...
        reportError(error, reader) {
            ...
            try {
                Zotero.logError(new Error(`${name}: ${message}`));
            }
            catch (_) { }
        ...
        async getPDFTestContext() {
            ...
            if (activeID)
                candidates.push(Zotero.Items.get(activeID));
            try {
                ...
                for (const item of selected) {
                    ...
                    if (!item?.isAttachment?.() && item?.getAttachments) {
                        for (const id of item.getAttachments())
                            candidates.push(Zotero.Items.get(id));
                    }
                }
            }
            catch (_) { }
            for (const item of candidates) {
                if (!item?.isAttachment?.() || seen.has(item.id))
                    continue;
                seen.add(item.id);
                const path = await item.getFilePathAsync?.();
                if (!path || !/\.pdf$/i.test(path))
        ...
        async testParserEnvironment(parser = null) {
            ...
            if (parser === "zotero") {
                if (!Zotero.PDFWorker?.getFullText)
                    throw new Error("当前 Zotero 未提供 PDFWorker.getFullText。 ");
            ...
            if (parser === "mineru-api") {
                const token = asString(this.pref("mineru.token", "")).trim();
                if (!token)
                    throw new Error("MinerU Token 未填写。 ");
                const base = asString(this.pref("mineru.baseURL", "https://mineru.net")).replace(/\/$/, "");
                ...
                if (status >= 500)
                    throw new Error(`MinerU 服务异常：HTTP ${status}`);
